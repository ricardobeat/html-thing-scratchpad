...
#define puts(...) std::cout << __VA_ARGS__ << std::endl
...
void RenderThread(sf::RenderWindow *window) {
    while (window->isOpen()) {
        ...
        // puts("VIEW " << view_size.x << "WIN " << window_size.x << "VS " << window_width);

        // int window_width = (int*) fbWidth;
        // int window_height = (int*) fbHeight;

        view.setSize(window_width, window_height);
        // window->setView(view = sf::View(sf::FloatRect(0.f, 0.f, window_width, window_height)));
